...
public record CancelNodeUserCommand(UserId Id): ICommand<bool>;

public class CancelNodeUserCommandHandler(
    IUserRepository userRepository,
    IMemoryCache memoryCache,
    ILogger<CancelNodeUserCommandHandler> logger
): ICommandHandler<CancelNodeUserCommand, bool>
{
    public async Task<bool> Handle(CancelNodeUserCommand request, CancellationToken cancellationToken)
    {
        logger.LogInformation("取消节点用户 Id:{Id}", request.Id);
